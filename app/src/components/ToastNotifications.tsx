      ...
      {toasts.map((toast) => (
        <div
          ...
          className={`pointer-events-auto px-5 py-3 rounded-2xl shadow-lg backdrop-blur-md text-sm font-medium flex items-center gap-2 animate-toast-in ${
            toast.type === 'success' ? 'bg-[#8B9E6B]/90 text-white' :
            toast.type === 'info' ? 'bg-[#C49A5C]/90 text-white' :
            toast.type === 'warning' ? 'bg-[#D4A96A]/90 text-white' :
            'bg-[#C47A5C]/90 text-white'
          }`}
        >
          <span aria-hidden="true">{toast.type === 'success' ? '✓' : toast.type === 'info' ? 'ℹ' : toast.type === 'warning' ? '⚠' : '✕'}</span>
          {toast.message}
          <button
